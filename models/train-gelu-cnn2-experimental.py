...
from module_summary import summary
from torch.autograd import Variable

from utils import to_gpu, Corpus, batchify, output_file_name, make_output_directory
from models3 import Seq2Seq2Decoder, Seq2Seq, GELU, MLP_G, MLP_Classify
# from models1 import Seq2Seq2Decoder
import shutil
import pdb

'''
Purpose: Improve grammar of output
Adjustments from train-GELU-cnn.py: model concats the unconvoluted input latent space representation to the convoluted output latent space rerpesentation in the discriminator before feeding through the fully connected layer
'''
...
args.outf = output_file_name(args.data_path, args.outf, "gelu_cnn_m2")
...
# write model layer summary
print("Writing model layers summary\n")
model_summary_outf = "{}/model_summary.txt".format(args.outf)
print("Output file is:\n{}".format(model_summary_outf))
# with open("{}/model_summary.txt".format(args.outf), 'w') as f:
...
class CNN_D(torch.nn.Module):
    ...
    def __init__(self, ninput, noutput, n_filters=256, activation="lrelu", gpu=False, dropout=0.3):
        ...
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=self.n_filters, padding = 1, kernel_size=3)
        self.bn1 = nn.BatchNorm1d(256)
        self.conv2 = nn.Conv1d(in_channels=self.n_filters, out_channels=self.n_filters, padding=2, kernel_size=5)
        self.bn2 = nn.BatchNorm1d(256)
        self.conv3 = nn.Conv1d(in_channels=self.n_filters, out_channels=1,padding = 1, kernel_size=3)
        self.bn3 = nn.BatchNorm1d(256)

        self.GELU = GELU()
        self.dropout = nn.Dropout(dropout)
        # flatten before feeding into fully connected
        self.fc = nn.Linear(2*self.n_filters*ninput, 1, bias=True)  # calculate size here,1 bias = True)


    def forward(self, x):
        x = x.unsqueeze(1)
        h1 = self.GELU(self.conv1(x))
        n, c, l = h1.shape
        h1 = self.bn1(h1.view(n,l,c).contiguous()).view(n,c,l).contiguous()
        h2 = self.GELU(self.conv2(h1))                
        n, c, l = h2.shape
        h2 = self.bn2(h2.view(n,l,c).contiguous()).view(n,c,l).contiguous()
        h3 = self.GELU(self.conv3(h2))
        n, c, l = h3.shape
        h3 = self.bn3(h3.view(n,l,c).contiguous()).view(n,c,l)
        combined = torch.cat([x,h3],0).view(-1).contiguous()
...
with open("{}/model.txt".format(args.outf), 'w') as f:
    f.write('autoencoder:\n')
    f.write(str(autoencoder))
    f.write('\n\n gan generator:\n')
    f.write(str(gan_gen))
    f.write('\n\n gan discriminator:\n')
    f.write(str(gan_disc))
    f.write('\n\n classifier:\n')
...
print("Writing Summary 2\n")
print("Writing Autoencoder\n")
summary(autoencoder, [[0],(1,),[1,args.batch_size]], args.batch_size, False, model_summary_outf, torch.LongTensor)
print("Wrote Autoencoder\n")
print("Writing Generator\n")
summary(gan_gen, (32,),args.batch_size, False,model_summary_outf)
print("Wrote Generator\n")
print("Writing Discriminator\n")
summary(gan_disc, (256,),args.batch_size, False,model_summary_outf)
print("Wrote Discriminator\n")
print("Writing Classifier\n")
summary(classifier,(256,),args.batch_size, False,model_summary_outf)
print("Wrote Classifier\n")
...
def train_classifier(whichclass, batch):
    ...
    # Train
    code = autoencoder(0, source, lengths, noise=False, encode_only=True,gpu=False).detach()
...
print('| end of epoch {:3d} | time: {:5.2f}s | test loss {:5.2f} | '
      ...
      format(epoch, (time.time() - epoch_start_time),
             test_loss, math.exp(test_loss), accuracy))
print('-' * 89)
endTime =  (datetime.datetime.now()).strftime("%m-%d-%y-%H%M")
