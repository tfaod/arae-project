...
from models3 import MLP_Classify, MLP_G, GELU, Seq2Seq2Decoder
import shutil
import pdb

'''
NOTE - this experimental file tests the module-summary.py  which outputs a summary of the pytorch model
'''
...
args.outf = output_file_name(args.data_path, args.outf, "gelu_cnn_model_summary")
...
class CNN_D(torch.nn.Module):
    ...
    def __init__(self, ninput, noutput, n_filters=256, activation="lrelu", gpu=False, dropout=0.3):
        ...
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=self.n_filters, kernel_size=3)
        self.bn1 = nn.BatchNorm1d(254)
        self.conv2 = nn.Conv1d(in_channels=self.n_filters, out_channels=self.n_filters, kernel_size=5, stride=2, padding=1)
        self.bn2 = nn.BatchNorm1d(126)
        self.conv3 = nn.Conv1d(in_channels=self.n_filters, out_channels=self.n_filters, kernel_size=3)
        self.bn3 = nn.BatchNorm1d(124)
        self.conv3 = nn.Conv1d(in_channels=self.n_filters, out_channels=1, kernel_size=3)
        
        self.gelu = GELU()
        self.dropout = nn.Dropout(dropout)
        # flatten before feeding into fully connected
        # this is 64*123 x 1
        dim = int(((self.n_filters - 6)/2 - 2)*self.ninput)
        self.fc = nn.Linear(124*ninput, 1, bias=True)  # calculate size here,1 bias = True)
...
#         print(h1.shape)
        ...
        n, c, l = h3.shape
        h3 = self.bn3(h3.view(n,l,c).contiguous())
        h3 = h3.view(-1).contiguous()
#         h3 = self.dropout(h3)
        logits=self.fc(h3)
...
# torch_summary(autoencoder, [(25,),0,[1]*args.batch_size], args.batch_size, "cuda", model_summary_outf)
# print("Wrote Autoencoder\n")
# torch_summary(gan_gen, (32,),args.batch_size, "cpu",model_summary_outf)
# print("Wrote Generator\n")
# torch_summary(gan_disc, (256,),args.batch_size, "cpu",model_summary_outf)
# print("Wrote Discriminator\n")
# torch_summary(classifier,(256,),args.batch_size, "cpu",model_summary_outf)
