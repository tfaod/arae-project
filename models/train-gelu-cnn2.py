...
from models3 import Seq2Seq2Decoder, Seq2Seq, MLP_D, GELU, MLP_G, MLP_Classify
import pdb
...
class CNN_D(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        n, c, l = h3.shape
        h3 = self.bn3(h3.view(n,l,c).contiguous()).view(-1).contiguous()

        combined = torch.cat([x.view(-1).contiguous(),h3],dim=0)
