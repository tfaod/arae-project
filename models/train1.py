...
from models1 import Seq2Seq2Decoder, Seq2Seq, CNN_D, MLP_G, MLP_Classify
import shutil
import pdb
...
pdb.set_trace()
gan_disc = CNN_D(ninput=args.nhidden, noutput=1, layers=args.arch_d)
pdb.set_trace()
...
def train_gan_d(whichdecoder, batch):
    ...
    real_hidden = autoencoder(whichdecoder, source,
                              lengths, noise=False, encode_only=True)
    ...
    gradient_penalty = calc_gradient_penalty(
        gan_disc, real_hidden.data, fake_hidden.data)
    ...
    while niter < len(train1_data) and niter < len(train2_data):
        ...
        for i in range(args.niters_ae):
            ...
            total_loss_ae1, start_time = \
                train_ae(1, train1_data[niter],
                         total_loss_ae1, start_time, niter)
            total_loss_ae2, _ = \
                train_ae(2, train2_data[niter],
                         total_loss_ae2, start_time, niter)
