...
#Set emsize to 300 for emsize and nhidden
    def __init__(self, emsize, nhidden, ntokens, nlayers, weight_matrix, noise_r=0.2,\
        ...
        self.init_weights(weight_matrix)

        if share_decoder_emb:
            self.embedding_decoder2.weight = self.embedding_decoder1.weight

    def init_weights(self,weight_matrix):
        initrange = 0.1

        # Initialize Vocabulary Matrix Weight TODO: CHANGE THESE
	...
	#self.embedding.weight.requires_grad  = False
        #self.embedding_decoder1.load_state_dict({'weight': weight_matrix})
        #self.embedding_decoder1.weight.requires_grad  = False
        #self.embedding_decoder2.load_state_dict({'weight': weight_matrix})
...
class Seq2Seq(nn.Module):
    ...
    def init_weights(self):
        ...
        # Initialize Vocabulary Matrix Weight TODO: RESET THESE ONES TOO TO USE GLOVE
...
def generate(autoencoder, gan_gen, z, vocab, sample, maxlen):
    """
    Assume noise is batch_size x z_size, PASS IN VOCAB From GLOVE
    """
    if type(z) == Variable:
        noise=z
