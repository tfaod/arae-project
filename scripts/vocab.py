...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        """
        word_ids = [[  [self.char2id[c] for c in ('{' + w + '}') ] for w in s] for s in sents]
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = torch.tensor(sents_t, dtype=torch.long, device = device) # shape (batch_size, max_sent_length , m_word)
