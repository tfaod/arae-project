...
#         print(self.filter_sizes)
        ...
        self.convs = nn.ModuleList([nn.Conv1d(in_channels=1, out_channels=self.noutput, padding=0, kernel_size=filter_size) for filter_size in self.filter_sizes])
        self.activation=nn.LeakyReLU(.2)
        self.dropout = nn.Dropout(0.3)
        # flatten before feeding into fully connected
        self.fc = nn.Linear(self.ninput*noutput*self.noutput, 1, bias=True)  # calculate size here,1 bias = True)
