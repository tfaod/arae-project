...
class Seq2Seq2Decoder(nn.Module):
    ...
    def init_hidden(self, bsz):
        ...
        z1 = to_gpu(self.gpu, zeros1)
        z2 = to_gpu(self.gpu, zeros2)
        return (z1, z2)
...
#         self.conv2 = nn.Conv1d(in_channels=self.n_filters, out_channels=self.n_filters, kernel_size=3)
...
#         # this is 64*123 x 1
#         self.fc = nn.Linear((self.n_filters - 6)*self.ninput, 1, bias=True)  # calculate size here,1 bias = True)
    ...
    def __init__(self, ninput, noutput, n_filters=256, activation="lrelu", gpu=False, dropout=0.3):
        ...
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=self.n_filters, kernel_size=3)
        self.bn1 = nn.BatchNorm1d(self.n_filters - 2)
        self.conv2 = nn.Conv1d(in_channels=self.n_filters, out_channels=self.n_filters, kernel_size=5, stride=2)
        self.bn2 = nn.BatchNorm1d(int((self.n_filters - 6)/2))
        self.conv3 = nn.Conv1d(in_channels=self.n_filters, out_channels=1, kernel_size=3)
        self.bn3 = nn.BatchNorm1d(int((self.n_filters - 6)/2 - 2))
        
        self.activation=ACTN_MAP[activation]
        self.dropout = nn.Dropout(dropout)
        # flatten before feeding into fully connected
        # this is 64*123 x 1
        totdim = int(self.ninput*((self.n_filters - 6)/2 - 2))
        self.fc = nn.Linear(totdim, 1, bias=True)  # calculate size here,1 bias = True)
...
class MLP_D(nn.Module):
    def __init__(self, ninput, noutput, layers, 
        ...
        layer_sizes = [ninput] + [int(x) for x in layers.split('-')]
        self.layers = []
        activation = ACTN_MAP[activation]
...
ACTN_MAP = {"lrelu": nn.LeakyReLU(0.2), "gelu": GELU()}
