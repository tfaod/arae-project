...
class CNN_D(torch.nn.Module):
    ...
    def __init__(self, ninput, noutput, filter_sizes='1-2-3-4-5', n_filters=256, activation=nn.LeakyReLU(0.2), gpu=False):
        ...
        self.conv4 = nn.Conv1d(in_channels=self.n_filters, out_channels=self.n_filters, kernel_size=5, stride=2)
        self.conv5 = nn.Conv1d(in_channels=self.n_filters, out_channels=1, kernel_size=3)
        
        self.activation=nn.LeakyReLU(.2)
        self.dropout = nn.Dropout(0.3)
        # flatten before feeding into fully connected
        self.fc = nn.Linear(self.ninput*(self.n_filters - 2), 1, bias=True)  # calculate size here,1 bias = True)
...
#         print("h1 shape is: {}".format(h1.shape))
        # h1 is shape (N, n_filters, l1)
        h2 = self.activation(self.conv2(h1))
        h2 = self.activation(self.conv3(h1))
        h2 = self.activation(self.conv4(h1))
        h2 = self.activation(self.conv5(h1))
#         print("h2 shape is: {}".format(h2.shape))
        # h1 is shape (N, n_filters, l2)
        h2 = h2.squeeze()
...
#             print("pool shape is: {}".format(pooled.shape))
        h2=h2.view(-1).contiguous()
#         print("h2 reshape is: {}".format(h2.shape))
        logits=self.fc(h2)
